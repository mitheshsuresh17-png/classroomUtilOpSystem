...
- **Scale target:** unknown — the college has not yet provided real room counts or department-specific data. Do not hardcode limits based on the current seed data (4 rooms). Build for "small-to-medium," not for huge scale, but don't paginate/optimize prematurely either.
- **Time slots are NOT an unknown — treat separately from room/department data.** Unlike room counts, the weekly time-slot grid (days × periods) is standard across the college and already known; it does not depend on pilot-department input. Do not wait on the college for this. Fold the full time-slot grid (currently only produced by running `server/seed_slots.cjs` separately, per the README's "Common Pitfalls" table) directly into `database/schema.sql`/`seed.sql` as part of Sprint 0, so a fresh setup has the complete, correct slot grid from the start — not just the 3 sample slots currently in `seed.sql`. Confirm the exact grid (days covered, period length, start/end times) if it isn't already unambiguous in the existing seed/slot script; do not guess new period boundaries.
...
- [ ] Fold the full time-slot grid (currently produced by the separate `seed_slots.cjs` script) into `database/seed.sql` directly. Time slots are fixed/known college-wide, not pilot-department-specific — do not treat this as blocked on the college. `seed_slots.cjs` should become redundant, not required, same as the other standalone deploy scripts above.
...
- [ ] Once the pilot department provides real room and course/batch numbers, replace seed data with their actual data (or a realistic placeholder set at that scale) and sanity-check performance/UX at that scale. Time slots should already be correct from Sprint 0 and do not need to wait on this.
...
- Real room and course/batch counts from the pilot department — needed before Sprint 5. (Time slots are already known college-wide and handled in Sprint 0 — not a blocker.)
